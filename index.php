  ...
  <div class="row mt-5">
    <div class="col-md-8">
      ...
      $sql = "SELECT posts.*, category.categoryName, users.name 
              FROM posts JOIN category ON posts.categoryID=category.categoryID 
              JOIN users ON posts.userID = users.userID ORDER BY postID DESC";
          ...
          if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              echo '<a href="customFeed.php?categoryID=' . $row['categoryID'] . '"
                    class="list-group-item list-group-item-action list-group-item-primary">' . $row['categoryName'] . '</a>';
            }
          }
          ?>
        </ul>
